...
bool TensorRT::constructNetwork(const std::string &onnxPath) {
	...
	mCudaEngine = std::shared_ptr<nvinfer1::ICudaEngine>(engine, common::InferDeleter());
...
bool TensorRT::serializeEngine(const std::string &save_path) {
    ...
    serialize_output_stream.open(save_path,std::ios::binary);
...
bool TensorRT::deseriazeEngine(const std::string &load_path) {
	...
	std::ifstream cache(load_path, std::ios::binary);
    if (!cache.good()){
        return false;
    }
    std::string deserialize_str;
    while (cache.peek() != EOF){ // 使用fin.peek()防止文件读取时无限循环
...
float TensorRT::infer(const std::vector<std::vector<float>> &InputDatas, common::BufferManager &bufferManager, cudaStream_t stream) const {
	...
	//将InputDatas数据复制到getHostBuffer时时连续的，包括对image batch的输入，一次性copy
...
StreamProcess::StreamProcess(DetectionTRT *trt, uint32_t len) : mTrt(trt), mQ1(len), mQ2(len), mQ3(len), mQ4(len), flag_done(false) {
